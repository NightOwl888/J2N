...
namespace J2N.Collections.ObjectModel.Tests
{
    ...
    /// are thrown.
    /// </summary>
    public class ReadOnlyListTests : CollectionTestBase
    {
        private static readonly ReadOnlyList<int> s_empty = new ReadOnlyList<int>(new int[0]);

        [Fact]
        public static void Ctor_NullList_ThrowsArgumentNullException()
        {
            AssertExtensions.Throws<ArgumentNullException>("list", () => new ReadOnlyList<int>(null));
        }
        ...
        {
            var collection = (IList<int>)new ReadOnlyList<int>(s_intArray);
        ...
        private class TestCollection<T> : ReadOnlyList<T>
        {
