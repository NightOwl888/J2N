...
namespace J2N.Numerics
{
    public class TestDouble : TestCase
    {
                ...
                string expectedString)
        {
            ...
            result = Double.ParseDouble(originalDoubleString, J2N.Text.StringFormatter.InvariantCulture);
        ...
        public void Test_parseDoubleLjava_lang_String()
        {
            assertEquals("Incorrect double returned, expected zero.", 0.0, Double
                    .ParseDouble("2.4703282292062327208828439643411e-324", J2N.Text.StringFormatter.InvariantCulture), 0.0);
            assertTrue("Incorrect double returned, expected minimum double.", Double
                    .ParseDouble("2.4703282292062327208828439643412e-324", J2N.Text.StringFormatter.InvariantCulture) == double.Epsilon); // J2N NOTE: MIN_VALUE in Java is the same as Epsilon in .NET

            for (int i = 324; i > 0; i--)
            {
                Double.ParseDouble("3.4e-" + i, J2N.Text.StringFormatter.InvariantCulture);
            }
            for (int i = 0; i <= 309; i++)
            {
                Double.ParseDouble("1.2e" + i, J2N.Text.StringFormatter.InvariantCulture);
            }
