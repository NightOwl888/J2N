#if FEATURE_METHODIMPLOPTIONS_INTERNALCALL
...
namespace J2N.Runtime.CompilerServices
{
        ...
        where TValue : class?
    {
        ...
        /// </summary>
        /// <remarks>
        /// The <see cref="ConditionalWeakTable{TKey, TValue}"/>  constructor instantiates an
        /// empty <see cref="ConditionalWeakTable{TKey, TValue}"/>  table; that is, the table contains
        /// no key/value pairs. You can add key/value pairs by calling the <see cref="Add(TKey, TValue)"/>,
        /// <see cref="GetValue(TKey, CreateValueCallback)"/>, or <see cref="GetOrCreateValue(TKey)"/> method.
        /// <para/>
        /// Every key in a <see cref="ConditionalWeakTable{TKey, TValue}"/> object must be unique.
        /// Keys are not unique if they refer to the same object (that is, if passing them as arguments
        /// to the <see cref="object.ReferenceEquals(object, object)"/> method returns <c>true</c>).
        /// </remarks>
...
#else
                    ((IEnumerable<KeyValuePair<TKey, TValue>>)new KeyValuePair<TKey, TValue>[0]).GetEnumerator() :
#endif
                    new Enumerator(this);
            }
        }
