// Source: https://github.com/dotnet/runtime/blob/v10.0.0-rc.2.25502.107/src/libraries/System.Private.CoreLib/src/System/Globalization/OrdinalCasing.Icu.cs
    ...
    internal static partial class OrdinalCasing
    {
        ...
        internal static void ToUpperOrdinal(ReadOnlySpan<char> source, Span<char> destination)
        {
            ...
            for (int i = 0; i < source.Length; i++)
            {
                ...
                destination[i] = char.ToUpperInvariant(c);
            }
        }
        ...
        internal static int CompareStringIgnoreCase(ref char strA, int lengthA, ref char strB, int lengthB)
        {
            ...
            Span<char> bufferA = stackalloc char[2];
            Span<char> bufferB = stackalloc char[2];
            Span<char> resultA = stackalloc char[2];
            Span<char> resultB = stackalloc char[2];
            ...
            while (index < length)
            {
                ...
                if (!char.IsHighSurrogate(a) || index >= lengthA - 1 || !char.IsLowSurrogate(lowSurrogateA = Unsafe.Add(ref charA, 1)))
                {
                    if (!char.IsHighSurrogate(b) || index >= lengthB - 1 || !char.IsLowSurrogate(Unsafe.Add(ref charB, 1)))
                    {
                        ...
                        char aUpper = char.ToUpperInvariant(a);
                        char bUpper = char.ToUpperInvariant(b);
                ...
                bufferA[0] = a;
                bufferA[1] = lowSurrogateA;

                bufferB[0] = b;
                bufferB[1] = lowSurrogateB;

                System.MemoryExtensions.ToUpperInvariant(bufferA, resultA);
                System.MemoryExtensions.ToUpperInvariant(bufferB, resultB);

                uint upperSurrogateA = UnicodeUtility.GetScalarFromUtf16SurrogatePair(resultA[0], resultA[1]);
                uint upperSurrogateB = UnicodeUtility.GetScalarFromUtf16SurrogatePair(resultB[0], resultB[1]);
