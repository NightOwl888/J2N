...
{
    internal static partial class FloatingPointNumber
    {
        ...
        internal unsafe ref struct BigInteger
        {
            ...
            public static void DivRem(ref BigInteger lhs, ref BigInteger rhs, out BigInteger quo, out BigInteger rem)
            {
                ...
                {
                    uint quotient = MathExtensions.DivRem(lhs._blocks[0], rhs._blocks[0], out uint remainder);
                ...
                if (rhsLength == 1)
                {
                    ...
                    for (int i = quoLength - 1; i >= 0; i--)
                    {
                        ...
                        ulong digit;
                        digit = MathExtensions.DivRem(value, rhsValue, out carry);
            ...
            public static void SetValue(out BigInteger result, ref BigInteger value)
            {
                ...
                //Array.Copy((uint[])result._blocks, )

                //fixed (uint* firstItemSource = &result._blocks[0], firstItemDestination = &value._blocks[0])
                //{
                //    //System.Runtime.InteropServices.Marshal.Copy(new IntPtr(result._blocks[0]), new IntPtr(value._blocks[0]), 0, rhsLength);
                //    Marshal.Copy(new IntPtr(firstItemSource), )
                //        }
            }
