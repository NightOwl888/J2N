...
namespace J2N.IO
{
    ...
    public abstract class DoubleBuffer : Buffer, IComparable<DoubleBuffer>
    {
        ...
        /// <remarks>Note to inheritors: This implementation reads <see cref="double"/>s one at a time and it is highly
        ...
        /// <exception cref="ArgumentNullException">If <paramref name="destination"/> is <c>null</c>.</exception>
        public virtual DoubleBuffer Get(double[] destination, int offset, int length) // J2N TODO: API - rename startIndex instead of offset
        {
            if (destination is null)
                ThrowHelper.ThrowArgumentNullException(ExceptionArgument.destination);
            if (offset < 0)
        ...
        /// <summary>
        /// Reads <see cref="double"/>s from the current position into the specified span,
        /// and increases the position by the number of <see cref="double"/>s read.
        /// <para/>
        /// The <see cref="Span{Double}.Length"/> property is used to determine
        /// how many <see cref="double"/>s to read.
        /// </summary>
        /// <param name="destination">The target span, sliced to the proper position, if necessary.</param>
        /// <returns>This buffer.</returns>
        /// <remarks>Note to inheritors: This implementation reads <see cref="double"/>s one at a time and it is highly
        /// recommended to override to provide a more optimized implementation.</remarks>
        /// <exception cref="BufferUnderflowException">If <see cref="Span{Double}.Length"/> is greater than
        /// <see cref="Buffer.Remaining"/>.</exception>
        public virtual DoubleBuffer Get(Span<double> destination) // J2N specific
        {
        ...
        /// <returns>This buffer.</returns>
        /// <remarks>Note to inheritors: This implementation writes <see cref="double"/>s one at a time and it is highly
        /// recommended to override to provide a more optimized implementation.</remarks>
        ...
        /// <exception cref="ReadOnlyBufferException">If no changes may be made to the contents of this buffer.</exception>
        /// <exception cref="ArgumentNullException">If <paramref name="source"/> is <c>null</c>.</exception>
        public virtual DoubleBuffer Put(double[] source, int offset, int length) // J2N TODO: API - rename startIndex instead of offset
        {
            if (source is null)
                ThrowHelper.ThrowArgumentNullException(ExceptionArgument.source);
            if (offset < 0)
        ...
        /// <summary>
        /// Writes <see cref="double"/>s in the given span to the current position and increases the
        /// position by the number of <see cref="double"/>s written.
        /// <para/>
        ...
        /// <exception cref="BufferOverflowException">If <see cref="Buffer.Remaining"/> is less than <c>source.Length</c>.</exception>
        /// <exception cref="ReadOnlyBufferException">If no changes may be made to the contents of this buffer.</exception>
        public virtual DoubleBuffer Put(ReadOnlySpan<double> source) // J2N specific
        {
        ...
        public virtual DoubleBuffer Put(DoubleBuffer source)
        {
            ...
            Span<double> contents = length * sizeof(double) > ByteStackBufferSize
                ? (arrayToReturnToPool = ArrayPool<double>.Shared.Rent(length)).AsSpan(0, length)
            ...
            {
                ArrayPool<double>.Shared.ReturnIfNotNull(arrayToReturnToPool);
            }
        }
