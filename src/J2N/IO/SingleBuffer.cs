...
namespace J2N.IO
{
    ...
    public abstract class SingleBuffer : Buffer, IComparable<SingleBuffer>
    {
        ...
        /// <remarks>Note to inheritors: This implementation reads <see cref="float"/>s one at a time and it is highly
        ...
        /// <summary>
        /// Reads <see cref="float"/>s from the current position into the specified span,
        /// and increases the position by the number of <see cref="float"/>s read.
        /// <para/>
        /// The <see cref="Span{Single}.Length"/> property is used to determine
        /// how many <see cref="float"/>s to read.
        /// </summary>
        /// <param name="destination">The target span, sliced to the proper position, if necessary.</param>
        /// <returns>This buffer.</returns>
        /// <remarks>Note to inheritors: This implementation reads <see cref="float"/>s one at a time and it is highly
        /// recommended to override to provide a more optimized implementation.</remarks>
        /// <exception cref="BufferUnderflowException">If <see cref="Span{Single}.Length"/> is greater than
        /// <see cref="Buffer.Remaining"/>.</exception>
        public virtual SingleBuffer Get(Span<float> destination) // J2N specific
        {
        ...
        public SingleBuffer Put(float[] source) => InternalPut(source);

        internal virtual SingleBuffer InternalPut(float[] source) // J2N: Hack so we can use span by default without affecting calling order
        {
            if (source is null)
        ...
        /// <returns>This buffer.</returns>
        /// <remarks>Note to inheritors: This implementation writes <see cref="float"/>s one at a time and it is highly
        /// recommended to override to provide a more optimized implementation.</remarks>
        ...
        /// <summary>
        /// Writes <see cref="float"/>s in the given span to the current position and increases the
        /// position by the number of <see cref="float"/>s written.
        /// <para/>
        ...
        /// <exception cref="BufferOverflowException">If <see cref="Buffer.Remaining"/> is less than <c>source.Length</c>.</exception>
        /// <exception cref="ReadOnlyBufferException">If no changes may be made to the contents of this buffer.</exception>
        public virtual SingleBuffer Put(ReadOnlySpan<float> source) // J2N specific
        {
        ...
        public virtual SingleBuffer Put(SingleBuffer source)
        {
            ...
            Span<float> contents = length * sizeof(float) > ByteStackBufferSize
                ? (arrayToReturnToPool = ArrayPool<float>.Shared.Rent(length)).AsSpan(0, length)
            ...
            {
                ArrayPool<float>.Shared.ReturnIfNotNull(arrayToReturnToPool);
            }
        }
