...
namespace J2N.IO
{
    ...
    public abstract class Int64Buffer : Buffer, IComparable<Int64Buffer>
    {
        ...
        /// <remarks>Note to inheritors: This implementation reads <see cref="long"/>s one at a time and it is highly
        ...
        /// <summary>
        /// Reads <see cref="long"/>s from the current position into the specified span,
        /// and increases the position by the number of <see cref="long"/>s read.
        /// <para/>
        /// The <see cref="Span{Int64}.Length"/> property is used to determine
        /// how many <see cref="long"/>s to read.
        /// </summary>
        /// <param name="destination">The target span, sliced to the proper position, if necessary.</param>
        /// <returns>This buffer.</returns>
        /// <remarks>Note to inheritors: This implementation reads <see cref="long"/>s one at a time and it is highly
        /// recommended to override to provide a more optimized implementation.</remarks>
        /// <exception cref="BufferUnderflowException">If <see cref="Span{Int64}.Length"/> is greater than
        /// <see cref="Buffer.Remaining"/>.</exception>
        public virtual Int64Buffer Get(Span<long> destination) // J2N specific
        {
        ...
        public Int64Buffer Put(long[] source) => InternalPut(source);

        internal virtual Int64Buffer InternalPut(long[] source) // J2N: Hack so we can use span by default without affecting calling order
        {
            if (source is null)
        ...
        /// <returns>This buffer.</returns>
        /// <remarks>Note to inheritors: This implementation writes <see cref="long"/>s one at a time and it is highly
        /// recommended to override to provide a more optimized implementation.</remarks>
        ...
        /// <summary>
        /// Writes <see cref="long"/>s in the given span to the current position and increases the
        /// position by the number of <see cref="long"/>s written.
        /// <para/>
        ...
        /// <exception cref="BufferOverflowException">If <see cref="Buffer.Remaining"/> is less than <c>source.Length</c>.</exception>
        /// <exception cref="ReadOnlyBufferException">If no changes may be made to the contents of this buffer.</exception>
        public virtual Int64Buffer Put(ReadOnlySpan<long> source) // J2N specific
        {
        ...
        public virtual Int64Buffer Put(Int64Buffer source)
        {
            ...
            Span<long> contents = length * sizeof(long) > ByteStackBufferSize
                ? (arrayToReturnToPool = ArrayPool<long>.Shared.Rent(length)).AsSpan(0, length)
            ...
            {
                ArrayPool<long>.Shared.ReturnIfNotNull(arrayToReturnToPool);
            }
        }
