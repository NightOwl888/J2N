...
namespace J2N.IO
{
    ...
    public abstract class ByteBuffer : Buffer, IComparable<ByteBuffer>
    {
        ...
        public ByteBuffer Put(byte[] source) => InternalPut(source);

        internal virtual ByteBuffer InternalPut(byte[] source) // J2N: Hack so we can use span by default without affecting calling order
        {
            if (source is null)
