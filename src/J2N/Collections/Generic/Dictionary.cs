...
#pragma warning restore IDE0079 // Remove unnecessary suppression
        ...
        /// </summary>
        /// <typeparam name="TAlternateKey">The alternate type of a key for performing lookups.</typeparam>
        /// <param name="lookup">The created lookup instance when the method returns <see langword="true"/>, or
        /// a default instance that should not be used if the method returns <see langword="false"/>.</param>
        ...
        public readonly struct AlternateLookup<TAlternateKey> where TAlternateKey : allows ref struct
        {
            ...
            /// <returns>A reference to the value associated with the key, if found; otherwise, a <see langword="null"/> reference.</returns>
        ...
        {
            /// <summary>Initialize the instance. The dictionary must have already been verified to have a compatible comparer.</summary>
            internal SpanAlternateLookup(Dictionary<TKey, TValue> dictionary)
            {
                Debug.Assert(dictionary is not null);
            ...
            /// <summary>Gets the dictionary's alternate comparer. The dictionary must have already been verified as compatible.</summary>
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            internal static ISpanAlternateEqualityComparer<TAlternateKeySpan, TKey> GetAlternateComparer(Dictionary<TKey, TValue> dictionary)
            {
                Debug.Assert(IsCompatibleKey(dictionary));
            ...
            internal ref TValue? GetValueRefOrAddDefault(ReadOnlySpan<TAlternateKeySpan> key, out bool exists)
            {
                ...
                while ((uint)i < (uint)entries!.Length) // [!]: asserted above
                {
                    if (entries[i].hashCode == hashCode && comparer!.Equals(key, entries[i].key!)) // [!]: asserted above
                    {
                        exists = true;
                ...
                actualKey = comparer!.Create(key); // [!]: asserted above
