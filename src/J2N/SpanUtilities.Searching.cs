...
namespace J2N
{
    ...
    public static partial class SpanUtilities
    {
        ...
        /// <exception cref="ArgumentNullException"><paramref name="value"/> is <c>null</c>.</exception>
        /// <exception cref="ArgumentOutOfRangeException"><paramref name="startIndex"/> is less than
        /// 0 (zero) or greater than the length of <paramref name="span"/>.</exception>
        /// <remarks>
        /// Index numbering starts from zero.
        /// <para/>
        /// This method performs an ordinal (culture-insensitive) search, where a character is considered
        ...
        {
            // J2N: Unlike the JDK which "auto-corrects" the startIndex parameter to be within range, we disallow invalid values.
            // This is primarily to match this existing check in J2N.Text.StringBuilderExtensions.
        ...
        /// <returns>The zero-based starting index position of value if that string is found, or -1
        /// if it is not found. If <paramref name="value"/> is <see cref="string.Empty"/>,
        /// it returns the lower value of either <paramref name="startIndex"/> or the length of
        ...
        public static int LastIndexOf(ReadOnlySpan<char> span, ReadOnlySpan<char> value, int startIndex)
        {
            ...
            // To match the JDK, allow search for empty string
            if (value.IsEmpty)
                return Math.Min(startIndex, span.Length);
